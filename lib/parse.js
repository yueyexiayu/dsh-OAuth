...
export function displayNameFromIdentity(body, depth = 0) {
  if (depth > 3 || !body) return null;
  if (typeof body === "string") return pickDisplayName(body);
  if (typeof body !== "object") return null;
  const direct = pickDisplayName(
    body.email,
    body.mail,
    body.userEmail,
  ...
  for (const nested of [body.user, body.profile, body.data, body.account]) {
    const found = displayNameFromIdentity(nested, depth + 1);
    if (found) return found;
  }
  if (!Array.isArray(body)) {
    for (const value of Object.values(body)) {
      if (typeof value === "string" && looksLikeEmail(value)) return value.trim();
    }
  }
  return null;
}
