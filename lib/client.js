window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function post(action, platform, sessionId) {
      return fetch(API_PATH, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ action: action, platform: platform, sessionId: sessionId || "" }),
      }).then(function (r) {
        return r.json();
      });
    }
    ...
    function DengluBar(props) {
      var sessionId = props && props.sessionId ? String(props.sessionId) : "";
      var state = React.useState({ isSupported: false, platform: null, account: null, pending: null, busy: false });
      var view = state[0];
      var setView = state[1];
            ...
            fetch(API_PATH + qs)
              ...
              .then(function (res) {
                ...
                setView(function (prev) {
                  return {
                    isSupported: Boolean(res.isSupported),
      ...
      function run(action, platform) {
        ...
        post(action, platform, sessionId)
          .then(function (res) {
            if (!res || !res.ok) {
              setView(function (prev) {
      ...
      if (!view.isSupported && !pending) return null;
      var item = null;
      if (view.platform === "grok" || view.platform === "gpt") {
        item = React.createElement(AccountItem, {
          label: view.platform === "grok" ? "Grok" : "GPT",
          platform: view.platform,
          acc: view.account || { loggedIn: false },
      ...
      return React.createElement("span", { className: "oauth-bar" }, item, pendingLine);
    }
    ...
    function apply(ctx) {
      ...
      ctx.slots.inject("conversation.composer.dock", function () {
        return ctx.slots.register(
          ...
          function (slotProps) {
            return React.createElement(DengluBar, { sessionId: slotProps && slotProps.sessionId });
          },
        );
      });
    }
